...
export function RegistrationReview({ data, onSubmit, onPrev, isSubmitting }: RegistrationReviewProps) {
  const primaryContact = data.step5?.contacts.find(c => c.is_primary) || data.step5?.contacts[0]
  const primaryTerritory = data.step4?.territories.find(t => t.is_primary) || data.step4?.territories[0]
